...
module "vault_hvd_eastus2" {
  ...
  # Networking
  #------------------------------------------------------------------------------
  vnet_id                         = module.vault_prereqs_east2.vnet_id
  vault_subnet_id                 = module.vault_prereqs_east2.vault_subnet_id
  lb_subnet_id                    = module.vault_prereqs_east2.vault_subnet_id
  lb_is_internal                  = true
  create_vault_private_dns_record = true
  private_dns_zone_name           = data.tfe_outputs.azure_core_infra_outputs.values.environment_info.eastus2.zone_name
  private_dns_zone_rg             = data.tfe_outputs.azure_core_infra_outputs.values.environment_info.eastus2.resource_group_name

  #------------------------------------------------------------------------------
  # Azure Key Vault installation secrets and unseal key
  #------------------------------------------------------------------------------
  prereqs_keyvault_rg_name               = data.tfe_outputs.azure_core_infra_outputs.values.environment_info.eastus2.resource_group_name
  prereqs_keyvault_name                  = module.vault_prereqs_east2.key_vault_name
  vault_license_keyvault_secret_id       = module.vault_prereqs_east2.vault_license_kv_secret_id
  vault_tls_cert_keyvault_secret_id      = module.vault_prereqs_east2.vault_cert_kv_secret_id
  vault_tls_privkey_keyvault_secret_id   = module.vault_prereqs_east2.vault_privkey_kv_secret_id
  vault_tls_ca_bundle_keyvault_secret_id = module.vault_prereqs_east2.vault_ca_bundle_kv_secret_id

  vault_seal_azurekeyvault_vault_name      = module.vault_prereqs_east2.key_vault_name
  vault_seal_azurekeyvault_unseal_key_name = azurerm_key_vault_key.vault_unseal_key_east2.name
